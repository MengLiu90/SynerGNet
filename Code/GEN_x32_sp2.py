...
seed_sample = random.randint(1, 500)
print('seed for sample', seed_sample)
sampled_df = pd.DataFrame(columns=df_aug.columns)
groups = df_aug.groupby(['track'])
for _, gp in groups:
    sampled_ins = gp.sample(n=32, random_state=seed_sample, replace=True)
    sampled_df = sampled_df.append(sampled_ins)
sampled_df.to_csv('/work/lmengm1/Test_aug/aug_samples_for_train/sampled_aug_data_for_each_org_ins_remedy_x32_sp2.csv', index=False)
df_sample = pd.read_csv('/work/lmengm1/Test_aug/aug_samples_for_train/sampled_aug_data_for_each_org_ins_remedy_x32_sp2.csv')
...
# directories
save_dir = '/work/lmengm1/Model_Selection/Final_model/results_GEN_x32_sp2'
if not os.path.isdir(save_dir):
    os.mkdir(save_dir)

test_dir = '/work/lmengm1/Model_Selection/Final_model/Data_test_GEN_x32_sp2'
if not os.path.isdir(test_dir):
    os.mkdir(test_dir)

train_dir = '/work/lmengm1/Model_Selection/Final_model/Data_train_GEN_x32_sp2'
if not os.path.isdir(train_dir):
    os.mkdir(train_dir)

testdata_dir_p = '/work/lmengm1/Model_Selection/Final_model/DataForModelTest_GEN_x32_sp2'
if not os.path.isdir(testdata_dir_p):
    os.mkdir(testdata_dir_p)

traindata_dir_p = '/work/lmengm1/Model_Selection/Final_model/DataForModelTrain_GEN_x32_sp2'
...
for train_index, test_index in skf.split(df, df.iloc[:, 5]):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.000001, weight_decay=0.1)
